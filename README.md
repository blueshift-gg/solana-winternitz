...
Constructions 3 and 6 of
[Drake, Khovratovich, Kudinov and Wagner, IACR CiC 2025](https://eprint.iacr.org/2025/055),
instantiated with SHA-256 at the paper's 128-bit classical and 64-bit
quantum level (Corollary 2, Parameter Requirements 2 and 3). Research
code, not audited.

| Instance | Signatures per key | Signature | Public key | Verify CU |
|---|---:|---:|---:|---:|
| `winternitz` | 1 | 864 B | 56 B | 29,524 |
| `xmss` | 256, one per leaf | 1,124 B | 56 B | 31,224 |
...
The crate is `no_std` with no on-chain dependencies. `verify` returns
...
const signer = Signer.create(xmss.SecretKey, 'tree.key', randomBytes(32)); // ~1 s: builds 256 leaves
...
const once = Signer.create(winternitz.SecretKey, 'once.key', randomBytes(32)); // one leaf
```
...
seed, the next leaf and the digest of the last message, so back up the
file, not the seed. `create` takes a seed that has never signed and
refuses an existing file; `open` takes only a file; one instance holds a
file at a time. `signAt` and `sign_at` sign under an explicit leaf and
record nothing; they exist for tests and vectors.

## Rules

- Put everything the program will act on in the message. The scheme binds
  the message, not the transaction around it.
...
|---|---:|---|
| Chains `v` | 35 | 35 × 4 bits = 140 ≥ 138, eq. (13) |
| Chain positions `2^w` | 16 | one nibble per chain |
| Target sum `T` | 325 | 200 verifier steps, ~940 salts per signature |
| Chain element `n` | 24 B | ≥ 183 bits at lifetime 2^8, eq. (15) |
| Public parameter `P` | 24 B | ≥ 142 bits, eq. (16) |
| Salt `ρ` | 24 B | ≥ 176 bits at lifetime 2^8 with 2^16 trials, eq. (14) |
| Tree height | 8 | 256 leaves; nodes are full 32-byte digests |

The bounds are what the authors'
[parameter script](https://github.com/b-wagn/hashsig-parameters) prints
for these inputs, and the tests pin them.
